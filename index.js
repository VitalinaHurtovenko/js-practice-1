...
function isPrimeNumber(num) {
  // Якщо число менше або дорівнює 1, то воно не є простим
  if (num <= 1) {
    return false;
  }
  ...
  for (let i = 2; i < num; i++) {
    ...
    if (num % i === 0) {
      // Якщо так, то num не є простим
      return false;
    }
  }

  // Якщо під час циклу не знайдено дільників, то num є простим
  return true;
}
...
function checkDataType(value) {
  ...
  return `Значення "${value}" має тип "${result}"`;
...
console.log("cube(3)", cube(5)); // Виведе: 27
